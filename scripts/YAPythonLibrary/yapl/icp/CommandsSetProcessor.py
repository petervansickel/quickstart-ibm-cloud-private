...
from yapl.aws.S3Helper import S3Helper
from yapl.ansible.AnsibleHelper import AnsibleHelper
from yapl.docker.DockerHelper import DockerHelper
...
class CommandHelper(object):
  ...
  def __init__(self, commandSetPath=None, intrinsicVariables=None, **restArgs):
    """
      Constructor
      
      commandSetPath      - path to directory that holds the command set definition yaml files
    ...
    if (not commandSetPath):
      raise MissingArgumentException("The command set path (commandSetPath) must be provided.")
    #endIf

    self.commandSetPath = commandSetPath
    ...
    self.variableValues = self.__initVariableValues(self.commandSetPath, **restArgs)
    self.metadata = self.__initMetaData(self.commandSetPath)
  ...
  def _createCommands_orig(self, cmdTemplates):
    """
  ...
  def createCommands_orig(self,commandPath):
    """
  ...
  def _invokeCommands(self,cmdTemplate):
    """
      Helper for invokeCommands.  Process one command template.
      
      A command template may define one command and the command artifacts.
      
      Or a command template may define multiple commands in multiple documents
      within the command template.
      
      The commands are executed in the order in which they are defined in the 
      template.
    """
    methodName = "_invokeCommands"
    ...
    baseName = os.path.basename(cmdTemplate)
    rootName,ext = os.path.splitext(baseName)
    cmdFilePath = os.path.join(stagingDir,"%s-command%s" % (rootName,ext))
    
    self.createCommandFile(commandFilePath=cmdFilePath,
                           templateFilePath=cmdTemplate,
                           parameters=self.variableValues
                          )
    ...
    for i in range(0,len(cmdDocs)):
      ...
      if (TR.isLoggable(Level.FINEST)):
        TR.finest(methodName,"Command doc [%d]:\n\t%s" % (i,doc))
      #endIf
      
      if (type(doc) != type({})):
        # This shouldn't happen
        TR.warning(methodName, "Skipping document that is not structured as a dictionary.")
        continue
      #endIf
      ...
      if (not kind or kind not in CommandNames):
        if (TR.isLoggable(Level.FINER)):
          TR.finer(methodName,"Unexpected kind: %s from: %s, skipping document:\n\t%s" % (kind,cmdFilePath,doc))
        #endIf
      else:            
        helperClass = CommandHelpers.get(kind)
        if (TR.isLoggable(Level.FINEST)):
        ...
        # Break out code path for different constructors here.
        if (kind == 's3'):
          helper = helperClass(region=self.AWSRegion)
        else:
          helper = helperClass()
        #endIf
        ...
        helper.invokeCommands(cmdDocs,i,stagingDirPath=stagingDir) 
  ...
  def invokeCommands(self, **restArgs):
    ...
    if (restArgs):
      ...
      if (TR.isLoggable(Level.FINEST)):
        cleaned = Scrubber.dreplace(self.variableValues,self.SensitiveVariables)
        TR.finest(methodName,"Using variable values: %s" % cleaned)
      #endIf
    #endIf
    
    try:
      if (self.metadata and not self.allIntrinsicsDefined()):
        TR.info(methodName,"Not all prerequisite intrinsic variables defined. Ignoring command set: %s" % self.commandSetPath)
      else:
            
        cmdTemplates = self.getYaml(self.commandSetPath,exclude=['variables','metadata'])
        
        if (not cmdTemplates):
          TR.warning(methodName,"No command template files found in: %s" % self.commandSetPath)
        else:
          
          if (len(cmdTemplates) > 1): cmdTemplates.sort()

          for template in cmdTemplates:
            self._invokeCommands(template)
          #endFor
        #endIf
      #endIf
...
class CommandSetProcessor(object):
  """
    Class to support the processing of command sets.
  """
  ...
  def __init__(self, commandSetsPath=None, intrinsicVariables=None, sensitiveVariables=None):
    """
      Constructor
    """
    object.__init__(self)
    ...
    if (not commandSetsPath):
      raise MissingArgumentException("A file system path to the command sets directory must be provided.")
    #endIf
  ...
  def processCommandSets(self):
    """
      Process all of the collections (sets) of commands defined in the commandSetsPath
      of the instance.
      ...
      execute can be cleaned up manually.
    """
    methodName = "processCommandSets"
    
    self.rc = 0
    commandSets = [x for x in os.listdir(self.commandSetsPath) if os.path.isdir(os.path.join(self.commandSetsPath,x))]
    if (not commandSets):
      TR.info(methodName,"No command set directories in: %s" % self.commandSetsPath)
    else:
      if (len(commandSets) > 1): commandSets.sort()
      for commandSet in commandSets:
        csPath = os.path.join(self.commandSetsPath,commandSet)
        try:
          TR.info(methodName,"STARTED executing command set at: %s" % csPath)
          cmdHelper = CommandHelper(csPath,intrinsicVariables=self.IntrinsicVariables,
